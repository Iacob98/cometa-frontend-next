...
export async function PUT(
  ...
  try {
    ...
    if (rooms_total !== undefined) updateFields.push(`rooms_total = ${parseInt(rooms_total)}`);
    if (beds_total !== undefined) updateFields.push(`beds_total = ${parseInt(beds_total)}`);
    if (rent_daily_eur !== undefined) updateFields.push(`rent_daily_eur = ${parseFloat(rent_daily_eur)}`);
    if (status !== undefined) updateFields.push(`status = '${status.replace(/'/g, "''")}'`);
